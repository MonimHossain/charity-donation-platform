...
export default function AdminNotificationsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetchAdminNotifications({ limit: "50" });
      setItems(res.items || []);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Admin alerts and activity</p>
        </div>
        <Button variant="outline" onClick={() => void markAllAdminNotificationsRead().then(load)}>
          Mark all read
        </Button>
      </div>
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  {!item.readAt && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => void markAdminNotificationRead(item.id).then(load)}
                    >
                      Mark read
                    </Button>
                  )}
