...
function ResetPasswordContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Reset failed. Please request a new link."));
    } finally {
      setLoading(false);
    }
  }
    ...
    <section className="min-h-[80vh] flex items-center justify-center py-12 px-6">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          ...
          <h1 className="font-serif text-2xl md:text-3xl text-primary">Reset your password</h1>
          ...
          {success ? (
            ...
            <div className="space-y-4 text-center text-sm text-muted-foreground">
              ...
              <Button asChild variant="outline" className="rounded-full">
                <Link href={buildAuthHref("/auth/forgot-password")}>Request a new link</Link>
              </Button>
            </div>
          ) : (
