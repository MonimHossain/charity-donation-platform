...
import MockBlogPost from "@/app/(public)/blog/[slug]/MockBlogPost";
import { useState, useEffect } from "react";
import { useParams } from "next/navigation";
import Link from "next/link";
import { Loader2, Calendar, Clock, ChevronRight, Copy, Check } from "lucide-react";
  ...
  if (notFound || !post) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
        ...
        <Link
          href="/blog"
          className="mt-6 rounded-full bg-purple-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-purple-700 transition-colors"
        >
          Back to Blog
        </Link>
      </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-purple-900 via-purple-800 to-purple-700 py-12 lg:py-16">
        ...
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="mt-6">
            ...
            {post.excerpt && <p className="mt-4 max-w-4xl text-lg leading-relaxed text-purple-200">{post.excerpt}</p>}
            ...
            {(post.tags || []).length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {(post.tags || []).map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-purple-500/30 bg-purple-800/40 px-3 py-1 text-xs font-medium text-purple-200"
                  >
                    {tag}
                  </span>
                ))}
